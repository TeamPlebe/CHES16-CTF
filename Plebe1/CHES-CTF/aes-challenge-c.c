...
void ark_sb_rnd(uint8_t* state, uint8_t round)
{
	uint8_t i, ark;
	for(i=0; i <16; i++){
		ark = (((state[i]^_stored_key[i+round*16]) ^ mask_key[i]) ^ mask_in) ^ mask_state[i];
		state[i] = (masked_sbox[ark] ^ mask_state[i]) ^ mask_out;
	}
}

void ark_sb(uint8_t* state, uint8_t round){
	uint8_t i;
	for(i=0; i <16; i++){
		state[i] = sbox[state[i]^_stored_key[i+round*16]];
	}
}
...
void aes(uint8_t *in){
	...
	ark_sb_asm(state, _stored_key, mask_state, mask_key, mask_in, mask_out, masked_sbox);
	shiftRows(state);shiftRows(mask_state);
	mixColumns(state);mixColumns(mask_state);
	// AES Rounds
	for(i = 0; i < 8; i++){
 		ark_sb_rnd(state,i+1);
		shiftRows(state);shiftRows(mask_state);
		mixColumns(state);mixColumns(mask_state);
	}
...
void aes_indep_key(uint8_t * key){
	...
	for (uint16_t i = 0; i < 256; i++){
		masked_sbox[i] = sbox[i ^ mask_in] ^ mask_out;
	}
}
